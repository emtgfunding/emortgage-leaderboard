...
const VICI_BASE        = process.env.VICI_BASE          || 'https://emortgage.talkitpro.com';
...
async function getViciCookie() {
  ...
  const r = await fetch(`${VICI_BASE}/vicidial/admin.php`, { method: 'POST', body, redirect: 'manual' });
...
app.get('/api/vici', async (req, res) => {
  try {
    ...
    // Parse the pipe-delimited pre block
    ...
    for (const line of text.split('\n')) {
      ...
      const name  = cols[1]; const id = cols[2];
      const group = cols[3]; const calls = parseInt(cols[5]);
...
app.get('/api/status', (req, res) => {
  res.json({
    sf_configured:   !!(SF_CLIENT_ID && SF_USERNAME),
